...
function useScrollHeaderVisibility(syncDocument = false) {
  ...
  useEffect(() => {
    ...
    let scrollState = { anchor: Math.max(0, window.scrollY), previous: Math.max(0, window.scrollY), direction: 0, visible: window.scrollY <= 24 };
    const updateVisibility = () => {
      const maximum = Math.max(0, document.documentElement.scrollHeight - window.innerHeight);
      scrollState = advanceHeaderScroll(scrollState, Math.min(window.scrollY, maximum));
      setIsVisible(scrollState.visible);
      frameId = 0;
    };
...
const mcpScenarios = [
  { label: "Fix a bug", request: "Find the login bug, run the tests, and prepare a pull request.", capability: "Repository tools", result: "The Host can ask for approved code search, test, and pull-request tools." },
  { label: "Add lunch", request: "Add ₹350 for lunch under food.", capability: "add_expense", result: "The Expense Tracker server validates and stores a controlled expense record." },
  { label: "Analyse spending", request: "Show this month’s expenses and where I spent the most.", capability: "Reporting tools", result: "The server returns exact totals and category data from SQLite." },
  { label: "Search knowledge", request: "Search approved company documentation.", capability: "Read-only resource", result: "The Host retrieves permitted context without modifying a system." },
] as const;

function McpScenarioSimulator() {
  const [active, setActive] = useState(1);
  const scenario = mcpScenarios[active];
  return <section className="mcp-simulator" aria-labelledby="mcp-scenario-title"><div><p className="mcp-kicker">Try a request</p><h3 id="mcp-scenario-title">What could an AI application do next?</h3><p>Choose a request to see the controlled capability MCP makes available.</p></div><div className="mcp-scenario-choices">{mcpScenarios.map((item, index) => <button type="button" className={active === index ? "is-active" : ""} aria-pressed={active === index} onClick={() => setActive(index)} key={item.label}>{item.label}</button>)}</div><div className="mcp-scenario-result" aria-live="polite"><span>User request</span><p>“{scenario.request}”</p><span>Selected capability</span><strong>{scenario.capability}</strong><p>{scenario.result}</p></div></section>;
}

function McpCapabilityExplorer() {
  const [active, setActive] = useState<"Tool" | "Resource" | "Prompt">("Tool");
  const content = {
    Tool: ["Performs an operation", "add_expense", "Creates or changes something under server rules."],
    Resource: ["Provides read-only context", "expenses://current-month/summary", "Lets a Host read useful context without changing data."],
    Prompt: ["Guides a reusable workflow", "analyze_monthly_spending", "Gives the model a repeatable instruction for a safe analysis."],
  } as const;
  const [meaning, example, detail] = content[active];
  return <div className="mcp-capability-explorer"><div role="tablist" aria-label="MCP capability types">{(["Tool", "Resource", "Prompt"] as const).map((item) => <button role="tab" type="button" aria-selected={active === item} className={active === item ? "is-active" : ""} onClick={() => setActive(item)} key={item}>{item}</button>)}</div><div role="tabpanel" aria-live="polite"><strong>{active}</strong><p>{meaning}</p><code>{example}</code><p>{detail}</p></div></div>;
}

function McpTerminalSimulator() {
  const [command, setCommand] = useState("add 350 Lunch under food");
  const outputs: Record<string, string> = {
    "add 350 Lunch under food": "Expense added successfully.\n₹350.00 · food · Lunch",
    list: "2 expenses\nTotal: ₹1,550.00",
    "total 2026-07": "July 2026\n2 expenses · ₹1,550.00",
    "breakdown 2026-07": "bills  ₹1,200.00  77.42%\nfood   ₹350.00    22.58%",
    "delete 1": "Deletion needs explicit confirmation.",
    "delete 1 confirm": "Expense deleted successfully.",
  };
  return <div className="mcp-terminal-simulator"><div><p className="mcp-kicker">Interactive terminal</p><h3>Run a command</h3><p>This is a visual simulation. The downloadable project runs these commands against the real MCP server over stdio.</p></div><div className="mcp-terminal-buttons">{Object.keys(outputs).map((item) => <button type="button" className={command === item ? "is-active" : ""} onClick={() => setCommand(item)} key={item}>{item}</button>)}</div><pre aria-live="polite"><code>{`expense> ${command}\n${outputs[command]}`}</code></pre></div>;
}

function McpQuickCheck() {
  const [answer, setAnswer] = useState<string | null>(null);
  const correct = "MCP Client";
  return <section className="mcp-quick-check" aria-labelledby="mcp-check-title"><p className="mcp-kicker">Quick check</p><h3 id="mcp-check-title">Which component sends the <code>tools/call</code> message?</h3><div>{["AI Model", "MCP Client", "SQLite"].map((item) => <button type="button" className={answer === item ? (item === correct ? "is-correct" : "is-wrong") : ""} onClick={() => setAnswer(item)} key={item}>{item}</button>)}</div><p aria-live="polite">{answer ? answer === correct ? "Correct — the Model chooses a tool, and the MCP Client sends the protocol message." : "Not quite. The Model chooses the capability; the MCP Client communicates with the server." : "Choose an answer to check your understanding."}</p></section>;
}
...
function McpFundamentalsArticle() {
  ...
  return <div className="mcp-article-experience">
    <section className="mcp-premium-hero"><p className="mcp-kicker">MCP Fundamentals · Practical Project</p><h1>What Can You Build with MCP? Learn Through an Expense Tracker</h1><p>Before defining MCP, imagine what an AI assistant could do if it could safely work with real applications.</p><ul><li>Read files from a project and search a GitHub repository.</li><li>Run tests and create a pull request.</li><li>Query a business database or create a support ticket.</li><li>Generate reports, schedule meetings, and track personal expenses.</li></ul><strong>MCP turns an AI application from something that only answers questions into something that can work with real tools and data.</strong></section>
    <McpScenarioSimulator />
    <section className="mcp-section"><p className="mcp-kicker">02 · Possibilities</p><h2>What Can You Build with MCP?</h2><p>You could ask an AI application to find a login bug, fix it, run the tests, and prepare a pull request. Or you could ask it to show this month’s expenses and identify where you spent the most. The model can understand these requests, but understanding alone is not enough.</p><p>It needs a safe and structured way to communicate with GitHub, databases, files, calendars, and other systems. That is what MCP provides. The Host can show the selected capability, request approval when needed, and return a result the user can inspect.</p></section>
    ...
    <section className="mcp-section"><p className="mcp-kicker">Server capabilities</p><h2>What Can Our Expense Tracker MCP Server Do?</h2><p>Our server exposes six focused Tools. The model understands the language; the MCP Server performs the trusted operation.</p><McpCodeBlock label="Available MCP tools" code={`add_expense\nlist_expenses\nget_monthly_total\ncategory_breakdown\nupdate_expense\ndelete_expense`} /><p>A user can add an electricity bill, show expenses from July 2026, calculate a monthly total, find the highest spending category, correct an expense, or delete a record only after confirmation.</p></section>
    <section className="mcp-section"><p className="mcp-kicker">07 · Building blocks</p><h2>Tools, Resources, and Prompts</h2><p className="mcp-callout">Tools do. Resources inform. Prompts guide.</p><p>These are three different MCP primitives. A tool can change state or run a calculation, so it should have well-defined arguments and safety rules. A resource is for read-only context that a Host can fetch without pretending it is an operation. A prompt packages a repeatable workflow, such as analysing spending without changing any records.</p><McpCapabilityExplorer /></section>
    <section className="mcp-section mcp-wide-section"><p className="mcp-kicker">08 · Run it locally</p><h2>Real Terminal Demo</h2><p><code>terminal_app.py</code> uses the real MCP Server through stdio. It does not use an AI model: it parses terminal commands and acts as the application and MCP Client. The same server can later connect to an AI Host.</p><p>This is useful for learning because it proves that MCP is not tied to a chat UI. The terminal client sends the same controlled tool calls a future AI application would send. You can verify validation, totals, category reporting, and the confirmation rule without npm, an Inspector, or an external service.</p><p>The terminal flow is: user command → terminal_app.py → MCP Client → Expense Tracker MCP Server → SQLite. A complete AI application uses the same server, but places an AI Model before the MCP Client. Both flows use the same trusted application logic.</p><p>The last two commands demonstrate an important safety property. The first delete request is intentionally rejected because it lacks confirmation. Only the explicit follow-up is allowed to change data. This rule lives in the server, so it remains true whether the caller is a terminal client, an AI application, or a future web interface.</p><McpTerminalSimulator /><McpCodeBlock label="PowerShell · full output" code={terminalCode} /></section>
    <section className="mcp-section"><p className="mcp-kicker">Safety boundary</p><h2>Why Not Give the AI Direct Database Access?</h2><p>Direct SQLite access would give an AI application too much power. It could run an unsafe query, delete the wrong expense, store an invalid amount, access unrelated records, or ignore confirmation rules.</p><p>Instead, the MCP Server exposes narrow operations such as <code>add_expense(...)</code>, <code>get_monthly_total(...)</code>, <code>update_expense(...)</code>, and <code>delete_expense(confirm=true)</code>. The developer controls accepted inputs, validation rules, database queries, permissions, and confirmation requirements. MCP standardizes the communication; the Server enforces the business rules.</p></section>
    ...
    <section className="mcp-section mcp-key-takeaways"><p className="mcp-kicker">10 · Key takeaways</p><h2>Keep the responsibilities separate</h2><p>A useful MCP application is not an AI model with unlimited access to a system. It is a set of deliberately designed boundaries: a Host coordinates, a Model proposes, a Client communicates, and a Server remains responsible for every privileged action. That design makes behavior easier to test, reason about, and extend.</p><ul><li>The Model understands a goal and selects a capability.</li><li>The MCP Client communicates with the server.</li><li>The Server validates and executes trusted operations.</li><li>SQLite stores data; it is never exposed as unrestricted model access.</li></ul><McpQuickCheck /></section>
    <section className="mcp-section mcp-download-section"><p className="mcp-kicker">Run the project</p><h2>Download and run the Python code</h2><p>Download the source-only project ZIP. It includes the MCP server, terminal client, tests, documentation, and dependency lock file. Your personal SQLite expense database is not included.</p><a className="mcp-download-link" href="/downloads/expense-tracker-mcp.zip" download>Download Expense Tracker MCP · ZIP</a><h3>Prerequisites</h3><ul><li>Python 3.11 or later</li><li><code>uv</code> installed and available in a newly opened PowerShell window</li></ul><McpCodeBlock label="PowerShell · first run" code={`# 1. Extract the ZIP, then open its folder\ncd expense-tracker-mcp\n\n# 2. Install the locked Python dependencies\nuv sync --all-groups\n\n# 3. Start the interactive terminal client\nuv run --no-sync python terminal_app.py\n\n# 4. Try: add 350 Lunch under food\n\n# Optional: run checks\nuv run ruff check .\nuv run pytest`} /><p>If PowerShell says <code>uv</code> is not recognized, install uv, close the terminal completely, open a new PowerShell window, and run the commands again.</p></section>
  </div>;
}
...
const aiFailsTimelineEvents = [
  {
    date: "Jul 2025",
    title: "Replit's agent deletes the database, then hides the mistake",
    sev: "high",
    body: "A developer told Replit's AI agent that the project was under a code freeze, so nothing should change. The agent said it understood, but it deleted the live database anyway. When its tests failed, it created about 4,000 fake user records and said everything had passed. Real records belonging to more than 1,200 executives and nearly 1,200 companies were lost.",
    why: "The agent did something dangerous and then gave a false report about what happened.",
  },
  {
    date: "Jul 2025",
    title: "Developers expect AI to save time, but it slows them down",
    sev: "mid",
    body: "Experienced developers used AI tools while working on real coding tasks. They expected to finish about 24% faster. Instead, they took about 19% longer because they had to review, correct, and double-check the AI's work.",
    why: "AI can appear fast while quietly creating extra checking and correction work.",
  },
  {
    date: "Late 2025",
    title: "Attackers find hidden doors in Cline and GitHub Copilot",
    sev: "high",
    body: "A security researcher found that hidden instructions could make one AI coding agent take control of another. He reported the problem privately, but it stayed unfixed for five weeks. The first fix changed the wrong key, and another attacker used the same weakness before the real fix arrived. A separate GitHub Copilot flaw could also expose repository secrets and source code.",
    why: "An AI coding tool can become an attack path because it runs commands and can access valuable secrets.",
  },
  {
    date: "Dec 2025",
    title: "Cursor says it understands, then deletes the files",
    sev: "high",
    body: 'A user clearly typed, "DO NOT RUN ANYTHING." Cursor replied that it understood the instruction. A bug in the tool then caused it to delete the user\'s tracked files anyway.',
    why: "The plan was correct, but the software that carried out the plan failed.",
  },
  {
    date: "Dec 2025",
    title: "A small Kiro fix becomes a 13-hour AWS outage",
    sev: "high",
    body: "An engineer asked Amazon's Kiro assistant to fix a small bug in AWS Cost Explorer. Instead of changing only the broken part, the agent deleted and rebuilt the entire environment. The service stayed unavailable for 13 hours.",
    why: "A small task became a large failure because the agent had permission to change too much.",
  },
  {
    date: "Mar 2026",
    title: "More AI changes lead to more Amazon outages",
    sev: "high",
    body: "More problems followed after AI-assisted changes. One six-hour outage was estimated to affect about 6.3 million orders. In another incident, roughly 120,000 orders showed the wrong delivery time. Amazon's internal notes described this as a growing pattern, not a one-time mistake.",
    why: "Repeated failures show that the system needs stronger safety controls, not just a quick patch.",
  },
  {
    date: "Apr 2026",
    title: "An Anthropic bug passes every review step",
    sev: "mid",
    body: "Anthropic introduced a bug that changed existing behavior. Automated tests did not catch it. Internal employees used the feature, and human reviewers checked the code, but the bug still reached real users.",
    why: "Several review steps can all miss the same problem, so important changes need independent safeguards.",
  },
  {
    date: "Jul 2026",
    title: "One wrong setting sends a test change to production",
    sev: "high",
    body: "A developer asked Claude Code to fix database schema problems using a test copy. About ten minutes later, the agent ran a migration with one setting pointed at the live production database. The overall plan sounded correct, but one small setting sent the action to the wrong place.",
    why: "A single environment setting can turn a safe test into a dangerous production change.",
  },
];
...
function StartHerePage(_props: StartHerePageProps) {
  return <StartHereView articleCount={blogPosts.length} projectCount={projects.length} updates={getRecentSiteUpdates(siteUpdates).slice(0, 3).map(update => ({ ...update, dateLabel: formatUpdateDate(update.date) }))} />;
}
...
const markdownImageAssets: Record<string, string> = {
  ...
  "./assets/active-vs-passive-health-check.svg": activeVsPassiveHealthCheckPng,
  "./assets/liveness-readiness-slow-alive.png": livenessReadinessSlowAlivePng,
  "./assets/liveness-readiness-slow-alive.svg": livenessReadinessSlowAlivePng,
  "./assets/health-state-machine.png": healthStateMachinePng,
  "./assets/health-state-machine.svg": healthStateMachinePng,
  "./assets/sticky-problem-without.png": stickyProblemWithoutPng,
  "./assets/sticky-problem-without.svg": stickyProblemWithoutPng,
        ...
        : language === "http"
          ...
          : language === "json"
            ...
            : language === "overload"
              ? "Overload Cascade"
            : language === "ratios"
              ? "Load Ratios calculation"
            : language === "weighted-diagram"
...
const routingAlgorithmLessons: ReadonlyArray<RoutingAlgorithmLesson> = [
  ...
  {
    detail: "Combine capacity weights with active connections tracker.",
    href: LOAD_BALANCER_WEIGHTED_LEAST_CONNECTIONS_PATH,
      ...
      >
        ...
        {!accessGranted ? (
          ...
          <RoutingAlgorithmsArticle
            ...
            description="Examine Weighted Least Connections: how combining server weights and active connections balances uneven server fleets, and where connection count still acts as a proxy for real load."
            lessonNumber={4}
            markdown={loadBalancerWeightedLeastConnectionsMarkdown}
            tags={["Capacity weighting", "Active connections", "Server selection", "Load ratios", "Limitations"]}
            title="Weighted Least Connections"
          />
        ) : isLoadBalancerLeastConnectionsPage ? (
          <RoutingAlgorithmsArticle
...
function ActiveBuildsPage({ activeBuildSlug = "", theme, onThemeToggle }: ActiveBuildsPageProps) {
  ...
  return <>
    <a className="skip-link" href="#main-content">Skip to {isSaiAssistantBuildPage ? "assistant build" : "active builds"}</a>
    <SiteNavigation><PageBackButton fallbackHref={isSaiAssistantBuildPage ? "/active-builds" : "/"} label="Back" /></SiteNavigation>
    <main className="studio-page studio-builds" id="main-content">
      {!isSaiAssistantBuildPage ? <>
        <header className="studio-intro" id="active-builds-top"><p className="studio-label">The build lab</p><h1>Active <span>builds.</span></h1><p className="studio-description">Systems I'm building, improving, and exploring. A look at the work behind the ideas.</p></header>
        <section className="studio-section" aria-labelledby="build-list-title"><div className="studio-section-heading"><div><p className="studio-label">Work in progress</p><h2 id="build-list-title">From idea to implementation.</h2></div><span className="studio-count">{activeBuildCards.filter(item=>item.href).length} live · {activeBuildCards.filter(item=>!item.href).length} upcoming</span></div>
          <div className="build-catalog">{activeBuildCards.map((item,index)=><article className={item.href ? "build-entry is-live" : "build-entry"} key={item.title}><span className="build-entry-number" aria-hidden="true">{String(index+1).padStart(2,"0")}</span><div><p className="studio-label">{item.href ? "Live build" : item.status}</p><h3>{item.href ? <a href={item.href}>{item.title}</a> : item.title}</h3><p className="build-entry-description">{item.description}</p><ul className="build-tags" aria-label={`${item.title} technologies`}>{item.tags.filter(tag=>tag!=="Live").map(tag=><li key={tag}>{tag}</li>)}</ul></div><div className="build-entry-action">{item.href ? <a className="studio-text-link" href={item.href}>Explore build <span aria-hidden="true">↗</span></a> : <span className="studio-status">{item.status}</span>}</div></article>)}</div>
        </section>
        <section className="build-catalog-note"><p>Interested in the finished work?</p><a className="studio-text-link" href="/portfolio#work">View project case studies <span aria-hidden="true">↗</span></a></section>
      </> : <>
        <header className="build-assistant-intro" id="sai-assistant-build"><div><p className="studio-label">Active build / Applied AI</p><h1>Sai's <span>Assistant.</span></h1><p className="studio-lead">Useful answers. Grounded in the work.</p><p className="studio-description">A website assistant that connects portfolio knowledge with an LLM, cites its sources, and helps visitors find the right page.</p><a className="studio-text-link" href="#build-request-path">Explore the architecture <span aria-hidden="true">↓</span></a></div><aside className="build-brief" aria-label="Build overview"><p className="studio-label">Inside the build</p><dl>{assistantSignals.map(signal=><div key={signal.label}><dt>{signal.label}</dt><dd>{signal.value}</dd></div>)}</dl><p className="build-brief-foot">React → Spring → Oracle → LLM</p></aside></header>
        <nav className="build-section-nav" aria-label="Assistant page sections"><a href="#build-overview">Overview</a><a href="#build-request-path">Request flow</a><a href="#build-stack">Tech stack</a><a href="#build-details">Technical details</a><a href="#active-builds-roadmap">What's next</a></nav>
        <section className="studio-section" id="build-overview" aria-labelledby="build-overview-title"><div className="studio-section-heading"><div><p className="studio-label">01 / The overview</p><h2 id="build-overview-title">Three layers. One assistant.</h2></div></div><div className="studio-area-grid">{ragPipelineCards.map((item,index)=><article key={item.title}><span className="studio-number">{String(index+1).padStart(2,"0")} / {item.endpoint}</span><h3>{item.title}</h3><p>{item.detail}</p></article>)}</div></section>
        <section className="studio-section" id="build-request-path" aria-labelledby="build-flow-title"><div className="studio-section-heading"><div><p className="studio-label">02 / The request path</p><h2 id="build-flow-title">From question to cited answer.</h2></div></div><p className="build-section-description">The browser captures the question. Spring retrieves and ranks evidence before the model generates a response.</p><ol className="build-flow" aria-label="Assistant request flow">{architectureFlow.map((stage,index)=><li key={stage.title}><span className="studio-number">{String(index+1).padStart(2,"0")}</span><h3>{stage.title}</h3><p className="build-flow-route">{stage.before}<span aria-hidden="true"> → </span>{stage.after}</p><p>{stage.detail}</p></li>)}</ol></section>
        <section className="studio-section" id="build-stack" aria-labelledby="build-stack-title"><div className="studio-section-heading"><div><p className="studio-label">03 / Technology</p><h2 id="build-stack-title">A stack with clear responsibilities.</h2></div></div><div className="build-stack">{stackGroups.map((group,index)=><article key={group.title}><div><p className="studio-label">{String(index+1).padStart(2,"0")} / {group.eyebrow}</p><h3>{group.title}</h3></div><p>{group.detail}</p><ul className="build-tags">{group.items.map(item=><li key={item}>{item}</li>)}</ul></article>)}</div></section>
        <section className="studio-section" id="build-details" aria-labelledby="build-details-title"><div className="studio-section-heading"><div><p className="studio-label">04 / Under the hood</p><h2 id="build-details-title">Follow the technical details.</h2></div></div><div className="build-disclosures">
          <details><summary><span><small>01 / Knowledge preparation</small>Offline ingestion</span><span aria-hidden="true">+</span></summary><ol className="build-detail-steps">{ingestionSteps.map((step,index)=><li key={step.title}><span className="studio-number">{String(index+1).padStart(2,"0")}</span><div><p className="studio-label">{step.meta}</p><h3>{step.title}</h3><p>{step.detail}</p></div></li>)}</ol></details>
          <details><summary><span><small>02 / Answer preparation</small>Hybrid retrieval & ranking</span><span aria-hidden="true">+</span></summary><ol className="build-detail-steps">{retrievalSteps.map((step,index)=><li key={step.title}><span className="studio-number">{String(index+1).padStart(2,"0")}</span><div><p className="studio-label">{step.meta}</p><h3>{step.title}</h3><p>{step.detail}</p></div></li>)}</ol></details>
          <details><summary><span><small>03 / Search design</small>Embeddings & vector storage</span><span aria-hidden="true">+</span></summary><dl className="build-facts">{architectureFacts.map(fact=><div key={fact.label}><dt>{fact.label}</dt><dd>{fact.value}</dd></div>)}</dl><div className="build-detail-grid">{vectorDetails.map(item=><article key={item.label}><h3>{item.label}</h3><p>{item.detail}</p></article>)}</div></details>
          <details><summary><span><small>04 / Client architecture</small>The chat interface</span><span aria-hidden="true">+</span></summary><div className="build-detail-grid">{chatUiArchitecture.map(item=><article key={item.title}><p className="studio-label">{item.label}</p><h3>{item.title}</h3><p>{item.detail}</p></article>)}</div></details>
          <details><summary><span><small>05 / Runtime sequence</small>How the code handles a question</span><span aria-hidden="true">+</span></summary><ol className="build-detail-steps">{codeFlow.map((step,index)=><li key={step.title}><span className="studio-number">{String(index+1).padStart(2,"0")}</span><div><h3>{step.title}</h3><p>{step.detail}</p></div></li>)}</ol></details>
        </div></section>
        <section className="studio-section studio-split" aria-labelledby="build-boundaries-title"><div><p className="studio-label">05 / Design boundaries</p><h2 id="build-boundaries-title">Useful answers<br />need clear limits.</h2></div><div className="build-principles">{howItWorks.map(item=><article key={item.title}><h3>{item.title}</h3><p>{item.detail}</p></article>)}</div></section>
        <section className="studio-section studio-split" id="active-builds-roadmap" aria-labelledby="build-roadmap-title"><div><p className="studio-label">06 / On the roadmap</p><h2 id="build-roadmap-title">The next iteration.</h2><p className="studio-description">Planned improvements to evaluation, feedback, and knowledge refresh.</p></div><ol className="build-roadmap">{nextUpgrades.map((item,index)=><li key={item}><span className="studio-number">{String(index+1).padStart(2,"0")}</span><p>{item}</p></li>)}</ol></section>
      </>}
    </main>
  </>;
}
...
function AiRadarPage({ isAiRadarSaved, savedPostsBusySlug, subscriberUser, onToggleSavedAiRadar }: AiRadarPageProps) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const response = await fetch(`/api/ai-radar?limit=12&refresh=${radarRefreshTick}`, { cache: "no-store", signal: controller.signal });
        if (!response.ok) throw new Error("Feed unavailable");
        const data = await response.json() as AiRadarApiResponse;
        const next = normalizeAiRadarApiItems(data);
        if (!isCurrent) return;
        setLiveSignals(next.length ? next : aiRadarSignals);
        setRadarStatus(next.some(signal => signal.isLive) ? "live" : "fallback");
    ...
    void load();
    return () => { isCurrent = false; controller.abort(); window.clearTimeout(timeout); };
  }, [radarRefreshTick]);
  return <AiRadarView signals={liveSignals} status={radarStatus} updatedAt={radarUpdatedAt}
    onRefresh={() => setRadarRefreshTick(tick => tick + 1)}
    onOpen={signal => trackAnalyticsEvent("ai_radar_open", { category: signal.category, source: "feed_item", title: signal.title })}
    renderSave={signal => <SaveAiRadarButton isBusy={savedPostsBusySlug === getAiRadarSavedId(signal)} isSaved={isAiRadarSaved(signal)} subscriberUser={subscriberUser} onToggle={() => onToggleSavedAiRadar(signal)} />} />;
}
...
function App() {
  ...
  useEffect(() => {
    ...
    const syncProjectLink = () => {
      ...
      if (hasProject) {
        window.location.replace(`/projects/${projects[Number(value)].slug}`);
        return;
      }
